...
| `model.providerDefaults.<providerId>` | string | `""` | - | - | Optional delegated-thread default model for one provider. Empty or omitted means agents must discover and submit a model with `thread_target_list` and `thread_create_batch`. |
