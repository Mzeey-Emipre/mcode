...
> **Custom desktop demos that seed data.** If you script your own Electron demo that creates workspaces or threads over RPC, launch with an isolated `MCODE_DATA_DIR` and read the launched server's `server.lock` from that dir for its port + auth token. Never scan a port range for a `/health` endpoint, because a blind scan can attach to another app sharing the range and write to the wrong database.
