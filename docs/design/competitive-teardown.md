...
- **Native `WebContentsView` bounds-sync loop.** `BrowserPanel.tsx`: a real `WebContentsView` (not iframe) kept pixel-aligned with the React shell via ResizeObserver + rAF burst on `transitionrun/end/cancel`. If Mcode's preview panel uses anything simpler, this is the correct pattern.
