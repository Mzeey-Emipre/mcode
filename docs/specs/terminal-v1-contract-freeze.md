...
| Renderer scrollback  | 100..5,000 lines, default 1,000. Legacy `0` migrates to 5,000.                                                                                                                                                |
...
interface PtyHostAdapter {
  ...
  inspectChildren(sessionId: UUID): Promise<{ hasChildren: boolean }>;
  close(input: {
    sessionId: UUID;
    hostGeneration: U64;
    closeSeq: U64;
...
| `CHECKPOINT_REJECTED`      | Checkpoint is stale, oversized, malformed, incomplete, duplicated with different bytes, or out of order. |
...
| `terminal.session.checkpoint.complete` | `{sessionId, attachmentId, attachmentEpoch, hostGeneration, uploadId, totalBytes, sha256}`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | `{accepted:true, checkpointThroughSeq}`                                               | `SESSION_NOT_FOUND=REATTACH`, `STALE_ATTACHMENT=REATTACH`, `STALE_HOST_GENERATION=REATTACH`, `CHECKPOINT_REJECTED=REATTACH`, `PROTOCOL_MISMATCH=RESTART`. Unknown result is `UNKNOWN_DELIVERY`; reattach and compare checkpoint sequence before retrying.                                                                                                                           |
...
| Client `0x04 checkpointChunk`       | session, attachment, upload; `primarySeq=chunkIndex`, `relatedSeq=chunkCount`          | 1..65,536 bytes                                                                              | `STALE_ATTACHMENT=REATTACH`, `STALE_HOST_GENERATION=REATTACH`, `CHECKPOINT_REJECTED=REATTACH`, `PROTOCOL_MISMATCH=RESTART`. A lost chunk is not assumed accepted; identical index/hash may be resent after the upload is reconciled.                                |
| Client `0x05 checkpointCompleteAck` | session, attachment, upload; `primarySeq=lastChunkIndex`, related `chunkCount`         | Empty                                                                                        | `STALE_ATTACHMENT=REATTACH`, `CHECKPOINT_REJECTED=REATTACH`, `PROTOCOL_MISMATCH=RESTART`. Delivery unknown is `UNKNOWN_DELIVERY`; call JSON `checkpoint.complete` only after all chunks are acknowledged locally.                                                   |
...
`primarySeq` is always the sequence the receiver must process next or confirm;
`relatedSeq` is only used where the row defines it. The receiver rejects a
duplicate with different bytes and accepts an identical duplicate only when the
operation is idempotent (output acknowledgements, checkpoint chunks by index
and hash).
...
bounded payload. Session messages carry `hostGeneration:U64`; `handshake`
carries `requestedGeneration:U64`; `probe` and `shutdown` carry the current
...
| `inspectChildren` | `{sessionId}`                                                                                                                                                                                                                                                                                                                                                                                                                   | `SESSION_NOT_FOUND=SAFE_RETRY`, `HOST_UNHEALTHY=SAFE_RETRY`, `PROTOCOL_MISMATCH=RESTART`. Unknown delivery is `SAFE_RETRY`.                                                                                                                   |
...
| `open`      | Chunk index equals next index, payload <=64 KiB, cumulative <= declared bytes | `open`      | Chunk retained in memory; duplicate same hash is idempotent.     |
| `open`      | Missing, out-of-order, oversized, stale, or conflicting duplicate chunk       | `rejected`  | Previous valid checkpoint remains; return `CHECKPOINT_REJECTED`. |
| `open`      | All chunks received and `checkpoint.complete` hash/size match                 | `installed` | Atomically replace checkpoint; expose `checkpointThroughSeq`.    |
| `open`      | Timeout, disconnect, or incomplete complete ack                               | `aborted`   | Do not replace prior checkpoint; reattach before retry.          |
| `installed` | Older or future checkpoint arrives                                            | `installed` | Ignore stale checkpoint; no replacement or success-shaped ack.   |
...
3. **Hide**: `C -> S detach(reason:hide)` after renderer writes settle; `C -> S checkpoint.begin/chunks/complete` when checkpoint is valid; `S` pauses delivery only; `H` continues output into replay; no lifecycle transition.
...
- Binary codec tests cover every frame kind, round trips, truncation, trailing bytes, wrong magic/version/kind/flags, wrong ID lengths, sequence overflow, payload overflow, duplicate bytes, and generation/attachment mismatches.
- Host protocol tests cover every server and host message, heartbeat cadence, malformed messages, stale generations, duplicate output, and bounded payloads.
...
- Settings migration tests cover legacy flat fields, legacy scrollback `0 -> 5000`, backup preservation, idempotence, atomic writes, malformed input, future versions, reset, workspace inheritance, rename survival, and deletion cascade.
