...
2. Run `check` for focused AgentService, resource-cleanup, and event tests, changed verification, and fast lint.
...
The resource-cleanup checks cover direct deletion with active descendants, managed worktree cleanup after an external directory loss, and teardown ownership. They use disposable repositories and in-memory state. The current public RPC API cannot create active handoff lineage without a provider turn, so that edge remains a focused-test proof.
