...
}: Pick<PullRequestDetailToolbarProps, "model" | "detail" | "viewAction" | "isNarrow" | "onClose" | "capabilities" | "mutationTransport" | "readTransport" | "onRefresh" | "onRefreshClick" | "refreshing" | "onFork" | "onForkInBackground" | "forkAllowed" | "forkUnavailableReason">) {
  ...
  return (
    ...
    >
      <div className="grid h-12 min-w-0 grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-stretch gap-4">
        ...
        <ToolbarActions model={model} detail={detail} viewAction={viewAction} isNarrow={isNarrow} onClose={onClose} capabilities={capabilities} mutationTransport={mutationTransport} readTransport={readTransport} onRefresh={onRefresh} onRefreshClick={onRefreshClick} refreshing={refreshing} onFork={onFork} onForkInBackground={onForkInBackground} forkAllowed={forkAllowed} forkUnavailableReason={forkUnavailableReason} />
      </div>
    </header>
  );
}
