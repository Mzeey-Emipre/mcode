...
export function NoticePrototype() {
  ...
  const details =
    scenario === "Working"
      ...
      : info[scenario];
  const detailPanel = !dismissed && detailsOpen && scenario !== "Working" && (
    ...
    <div className="flex min-h-0 min-w-0 flex-1 flex-col bg-background text-foreground">
      <div className="flex flex-wrap items-center gap-3 border-b border-border px-5 py-3">
        ...
        <label className="ml-auto flex items-center gap-2 text-xs text-muted-foreground">
          ...
          <select
            value={scenario}
            onChange={(event) => chooseScenario(event.target.value as Scenario)}
      ...
      <div className="flex min-h-0 flex-1">
            ...
            {variant === "B" ? (
              ...
              <div className="mt-3 rounded-xl border border-border bg-background p-3 shadow-sm">
                ...
                <div className="flex items-center justify-between border-t border-border pt-2 text-xs text-muted-foreground">
                  <span>
                    Model · {scenario === "Model changed" ? "GPT-5" : "Auto"}
                  </span>
                </div>
              </div>
            )}
        ...
        {variant === "C" && !dismissed && detailsOpen && scenario !== "Working" && (
