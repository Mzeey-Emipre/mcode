...
import { isComposerProviderNotice } from "../notices/provider-notices";
...
export function SessionDiagnostics({ threadId }: { readonly threadId: string }) {
  ...
  const diagnostics = notices?.filter((notice) =>
    notice.systemNotice?.scope === "session" && !isComposerProviderNotice(notice)) ?? [];
  if (!diagnostics.length) return null;
  const hasSecurityWarning = diagnostics.some((notice) => notice.systemNotice?.kind === "security");
  return (
    <details open={hasSecurityWarning} className="mx-4 mt-2 rounded-md border border-border px-3 py-2 text-xs">
      <summary className="cursor-pointer font-medium">{hasSecurityWarning ? "Session security warnings and diagnostics" : "Session diagnostics"} ({diagnostics.length})</summary>
      <ul className="mt-2 max-h-48 space-y-2 overflow-auto" aria-label="Session diagnostics">
        {diagnostics.map((notice) => {
