...
function availableMergeMethods(node: GithubPullRequestDetailNode): PullRequestMergeMethod[] {
  const methods: PullRequestMergeMethod[] = [];
  if (node.repository.mergeCommitAllowed) methods.push("merge");
  if (node.repository.squashMergeAllowed) methods.push("squash");
  if (node.repository.rebaseMergeAllowed) methods.push("rebase");
  return methods;
}
...
function defaultMergeMethod(
  ...
  return mergeMethods.includes(providerDefault) ? providerDefault : mergeMethods[0] ?? null;
}

function normalizeDetailMergeability(
  mergeable: GithubPullRequestDetailNode["mergeable"],
): PullRequestDetail["mergeability"] {
  if (mergeable === "MERGEABLE") return "mergeable";
  return mergeable === "CONFLICTING" ? "conflicting" : "unknown";
}

function normalizeDetailReviewDecision(
  reviewDecision: GithubPullRequestDetailNode["reviewDecision"],
): PullRequestDetail["reviewDecision"] {
  if (reviewDecision === "APPROVED") return "approved";
  if (reviewDecision === "CHANGES_REQUESTED") return "changes_requested";
  return reviewDecision === "REVIEW_REQUIRED" ? "review_required" : "unknown";
}

function githubPullRequestDetailCandidate(
  node: GithubPullRequestDetailNode,
  identity: PullRequestIdentity,
  url: string,
  latestCommit: NonNullable<GithubPullRequestDetailNode["commits"]["nodes"][number]>["commit"],
  ...
  return {
    ...
    mergeability: normalizeDetailMergeability(node.mergeable),
    mergeMethods,
    defaultMergeMethod: selectedMergeMethod,
    viewerCanBypassMergeRequirements: node.viewerCanMergeAsAdmin,
    reviewDecision: normalizeDetailReviewDecision(node.reviewDecision),
...
export function normalizeGithubPullRequestDetail(
  ...
  const mergeMethods = availableMergeMethods(node);
  const selectedMergeMethod = defaultMergeMethod(node, mergeMethods);
  if (!selectedMergeMethod) return null;
  const item = githubPullRequestDetailCandidate(
    node,
    identity,
    url,
    latestCommit,
...
const githubCheckConclusionStates: Record<string, PullRequestCheck["state"]> = {
  SUCCESS: "passing",
  FAILURE: "failing",
  ERROR: "failing",
  TIMED_OUT: "failing",
...
function normalizeGithubCheckConclusion(conclusion: string | null): PullRequestCheck["state"] {
  return conclusion === null ? "unknown" : githubCheckConclusionStates[conclusion] ?? "unknown";
}
...
export function normalizeGithubReviewThread(
  ...
  const comments = normalizeGithubReviewThreadComments(thread);
  const firstComment = comments[0];
  if (!firstComment) return null;
  const validated = PullRequestConversationItemSchema().safeParse(
    githubReviewThreadCandidate(thread, comments, firstComment, headOid),
  );
  return validated.success ? validated.data : null;
}

function normalizeGithubReviewThreadComments(
...
function latestGithubReviewThreadCommentUpdate(
  comments: ReturnType<typeof normalizeGithubReviewThreadComments>,
  firstComment: ReturnType<typeof normalizeGithubReviewThreadComments>[number],
): string {
  return comments.reduce(
    (latest, comment) => Date.parse(comment.updatedAt) > Date.parse(latest) ? comment.updatedAt : latest,
    firstComment.updatedAt,
  );
}

function githubReviewThreadCandidate(
  thread: z.infer<typeof githubReviewThreadSchema>,
  comments: ReturnType<typeof normalizeGithubReviewThreadComments>,
  firstComment: ReturnType<typeof normalizeGithubReviewThreadComments>[number],
  headOid: string,
  ...
  return {
    ...
    commitOid: githubReviewThreadCommitOid(thread),
    headOid,
    isResolved: thread.isResolved,
    isOutdated: thread.isOutdated,
    createdAt: firstComment.createdAt,
    updatedAt: latestGithubReviewThreadCommentUpdate(comments, firstComment),
    totalCount: thread.comments.totalCount,
    comments,
  };
}

function githubReviewThreadCommitOid(
  thread: z.infer<typeof githubReviewThreadSchema>,
): string | null {
  const comment = thread.comments.nodes[0];
  return comment?.commit?.oid ?? comment?.originalCommit?.oid ?? null;
}
...
export function conversationBoundedData(
  ...
  if (githubConversationExceedsTextLimit(input)) return { reason: "byte_limit" };
  const parsed = githubReviewThreadSchema.safeParse(input);
...
function githubConversationExceedsTextLimit(input: unknown): boolean {
  if (typeof input !== "object" || input === null) return false;
  const record = input as Record<string, unknown>;
  return textExceedsDetailLimit(record.body) || embeddedCommentExceedsDetailLimit(record.comments);
}

function textExceedsDetailLimit(value: unknown): boolean {
  return typeof value === "string" && value.length > PULL_REQUEST_DETAIL_TEXT_MAX_LENGTH;
}

function embeddedCommentExceedsDetailLimit(value: unknown): boolean {
  if (typeof value !== "object" || value === null) return false;
  const nodes = (value as Record<string, unknown>).nodes;
  return Array.isArray(nodes) && nodes.some(commentExceedsDetailLimit);
}

function commentExceedsDetailLimit(value: unknown): boolean {
  if (typeof value !== "object" || value === null) return false;
  return textExceedsDetailLimit((value as Record<string, unknown>).body);
}
...
export function normalizeGithubTimelineNode(
  ...
  const normalize = timelineNodeNormalizers[base.data.__typename];
  if (!normalize) return null;
  const candidate = normalize(input as Record<string, unknown>, headOid);
  if (!candidate) return null;
  const validated = PullRequestTimelineItemSchema().safeParse(candidate);
  return validated.success ? validated.data : null;
}

type TimelineNodeNormalizer = (node: Record<string, unknown>, headOid: string) => unknown | null;

const timelineNodeNormalizers: Partial<Record<
  z.infer<typeof githubTimelineBaseSchema>["__typename"],
  TimelineNodeNormalizer
>> = {
  PullRequestCommit: normalizeGithubTimelineCommit,
  PullRequestReview: normalizeGithubTimelineReview,
  IssueComment: normalizeGithubTimelineIssueComment,
  PullRequestReviewThread: normalizeGithubTimelineReviewThread,
  ConvertToDraftEvent: (node) => normalizeGithubTimelineLifecycle(node, "ConvertToDraftEvent"),
  ReadyForReviewEvent: (node) => normalizeGithubTimelineLifecycle(node, "ReadyForReviewEvent"),
  ClosedEvent: (node) => normalizeGithubTimelineLifecycle(node, "ClosedEvent"),
  ReopenedEvent: (node) => normalizeGithubTimelineLifecycle(node, "ReopenedEvent"),
  ReviewRequestedEvent: (node) => normalizeGithubTimelineReviewRequest(node, "ReviewRequestedEvent"),
  ReviewRequestRemovedEvent: (node) => normalizeGithubTimelineReviewRequest(node, "ReviewRequestRemovedEvent"),
  MergedEvent: normalizeGithubTimelineMerge,
};
...
function normalizeGithubTimelineCommit(node: Record<string, unknown>): unknown | null {
  ...
  return { kind: "commit", providerNodeId: commit.id, occurredAt: commit.committedDate,
    actor: normalizeGithubActor(commit.author?.user), url: normalizeGithubHttpUrl(commit.url),
    oid: commit.oid, messageHeadline: commit.messageHeadline.slice(0, 512) };
}
...
function normalizeGithubTimelineReview(node: Record<string, unknown>): unknown | null {
  ...
  return { kind: "review", providerNodeId: parsed.data.id, occurredAt,
    actor: normalizeGithubActor(parsed.data.author), url: normalizeGithubHttpUrl(parsed.data.url),
    state: normalizeReviewState(parsed.data.state), body: parsed.data.body.slice(0, PULL_REQUEST_DETAIL_TEXT_MAX_LENGTH),
    commitOid: parsed.data.commit?.oid ?? null };
}

function normalizeGithubTimelineIssueComment(node: Record<string, unknown>): unknown | null {
  const parsed = githubCommentSchema.safeParse(node);
  if (!parsed.success) return null;
  return { kind: "issue_comment", providerNodeId: parsed.data.id, occurredAt: parsed.data.createdAt,
    actor: normalizeGithubActor(parsed.data.author), url: normalizeGithubHttpUrl(parsed.data.url),
    body: parsed.data.body.slice(0, PULL_REQUEST_DETAIL_TEXT_MAX_LENGTH), updatedAt: parsed.data.updatedAt };
}

function normalizeGithubTimelineReviewThread(
  node: Record<string, unknown>,
  headOid: string,
): unknown | null {
  const thread = normalizeGithubReviewThread(node, headOid);
  if (!thread || thread.kind !== "review_thread") return null;
  return { kind: "review_thread", providerNodeId: thread.providerNodeId, occurredAt: thread.createdAt,
    actor: thread.comments[0]?.author ?? null, url: thread.comments[0]?.url ?? null,
    path: thread.path, line: thread.line, startLine: thread.startLine, side: thread.side,
    startSide: thread.startSide, originalLine: thread.originalLine, originalStartLine: thread.originalStartLine,
    subjectType: thread.subjectType, commitOid: thread.commitOid, headOid: thread.headOid,
    isResolved: thread.isResolved, isOutdated: thread.isOutdated, totalCount: thread.totalCount, comments: thread.comments };
}

const githubLifecycleTimelineSchema = z.object({
  id: z.string().min(1).max(256), createdAt: z.string().datetime({ offset: true }),
  actor: githubActorSchema.nullable(), url: z.string().max(4_096).nullable().optional(),
});

function normalizeGithubTimelineLifecycle(
  node: Record<string, unknown>,
  type: "ConvertToDraftEvent" | "ReadyForReviewEvent" | "ClosedEvent" | "ReopenedEvent",
): unknown | null {
  const parsed = githubLifecycleTimelineSchema.safeParse(node);
  if (!parsed.success) return null;
  if (type === "ConvertToDraftEvent" || type === "ReadyForReviewEvent") {
    return { kind: "readiness", providerNodeId: parsed.data.id, occurredAt: parsed.data.createdAt,
      actor: normalizeGithubActor(parsed.data.actor), url: normalizeGithubHttpUrl(parsed.data.url),
      readiness: type === "ConvertToDraftEvent" ? "draft" : "ready" };
  }
  return { kind: type === "ClosedEvent" ? "closed" : "reopened", providerNodeId: parsed.data.id,
    occurredAt: parsed.data.createdAt, actor: normalizeGithubActor(parsed.data.actor),
    url: normalizeGithubHttpUrl(parsed.data.url) };
}

const githubReviewRequestTimelineSchema = z.object({
  id: z.string().min(1).max(256), createdAt: z.string().datetime({ offset: true }),
  actor: githubActorSchema.nullable(), requestedReviewer: githubReviewerTargetSchema.nullable(),
});

function normalizeGithubTimelineReviewRequest(
  node: Record<string, unknown>,
  type: "ReviewRequestedEvent" | "ReviewRequestRemovedEvent",
): unknown | null {
  const parsed = githubReviewRequestTimelineSchema.safeParse(node);
  if (!parsed.success) return null;
  const reviewer = normalizeGithubReviewerTarget(parsed.data.requestedReviewer);
  if (!reviewer) return null;
  return { kind: type === "ReviewRequestedEvent" ? "review_requested" : "review_request_removed",
    providerNodeId: parsed.data.id, occurredAt: parsed.data.createdAt,
    actor: normalizeGithubActor(parsed.data.actor), url: null, reviewer };
}
...
function normalizeGithubTimelineMerge(node: Record<string, unknown>): unknown | null {
  ...
  return { kind: "merged", providerNodeId: parsed.data.id, occurredAt: parsed.data.createdAt,
    actor: normalizeGithubActor(parsed.data.actor), url: normalizeGithubHttpUrl(parsed.data.url),
    commitOid: parsed.data.commit?.oid ?? null, refName: parsed.data.mergeRefName?.slice(0, 255) ?? null };
}
