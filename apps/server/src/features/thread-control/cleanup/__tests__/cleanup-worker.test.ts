  ...
  describe("poll", () => {
    it("logs retention admission, selected kinds, and remaining due backlog", async () => {
      ...
      try {
        ...
        const infoCalls = info.mock.calls as unknown as Array<[string, Record<string, unknown>]>;
        const batchLog = infoCalls.find(([message]) => message === "CleanupWorker batch selected");
    ...
    it("logs deferred and blocked retention outcomes with bounded reasons", async () => {
      ...
      try {
        ...
        const deferredInfoCalls = info.mock.calls as unknown as Array<[string, Record<string, unknown>]>;
        const deferred = deferredInfoCalls.find(([message]) => message === "CleanupWorker job deferred");
        ...
        const blockedInfoCalls = info.mock.calls as unknown as Array<[string, Record<string, unknown>]>;
        const blocked = blockedInfoCalls.find(([message]) => message === "CleanupWorker job blocked");
    ...
    it("removes an expired named-branch thread while preserving a shared branch", async () => {
      ...
      expect(mockGitService.assessWorktreeRemovalSafety).not.toHaveBeenCalled();
    ...
    it("includes job kind and retry state in lifecycle logs", async () => {
      ...
      try {
        ...
        const infoCalls = info.mock.calls as unknown as Array<[string, Record<string, unknown>]>;
        const started = infoCalls.find(([message]) => message === "CleanupWorker job started");
        const completed = infoCalls.find(([message]) => message === "CleanupWorker job completed");
        ...
        const warnCalls = warn.mock.calls as unknown as Array<[string, Record<string, unknown>]>;
        const failed = warnCalls.find(([message]) => message === "CleanupWorker job failed, scheduled for retry");
