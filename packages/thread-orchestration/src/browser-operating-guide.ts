...
4. Inspect: browser_inspect is authoritative for readiness, tabs, capabilities, constraints, diagnostics, revision, and observationRef. Inspect before the first action unless browser_open returned sufficient snapshot evidence. Tool discovery alone does not prove readiness.
5. Target: use a semanticId from the latest inspection. Next prefer role plus accessibleName. Use CSS or coordinates only when no semantic target exists. Never guess an ambiguous target.
6. Act: browser_act requires the latest observationRef, a fresh idempotencyKey, deadlineMs, and a non-empty steps array. Never send \`steps: []\`. Canonical click: \`{ "observationRef": "<latest>", "idempotencyKey": "<fresh>", "deadlineMs": 30000, "steps": [{ "operation": "click", "target": { "semanticId": "<from-latest-inspect>" } }] }\`. Required fields include wait durationMs, click target, and assert text/target/url. Use at most eight steps and 60 seconds.
7. Verify: stop on failure, interruption, deadline, or invalidation. Verify non-navigation effects with an assert step or a fresh inspection. Navigation and reload end the batch. Inspect again before another mutation.
