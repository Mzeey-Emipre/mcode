...
- Intent: use the mcode-browser MCP server for Mcode's shared Preview lifecycle. A target may run in the background while the Browser panel is hidden, and the user can reveal that same tab later. Do not launch a separate browser process or automation profile, and never compete with direct user input.
- Target selection: browser_open creates one agent-owned background tab, makes it the sticky current tab, and returns its initial observation. Use browser_tabs to select or claim another tab and to release, close, or finalize controlled tabs. For page elements, prefer semanticId from the latest observation, then role plus accessibleName; use CSS selectors or coordinates only when the observation provides no semantic target.
