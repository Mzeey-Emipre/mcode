...
- Safe sequencing: browser_act input must include latest observationRef, fresh idempotencyKey, deadlineMs, and a non-empty steps array. Required fields include wait durationMs, click target, and assert text/target/url. Limit batches to eight steps and 60 seconds. Stop on failure, interruption, deadline, or invalidation; verify non-navigation effects with assert or fresh inspect. Navigation/reload end the batch; inspect before the next mutation.
