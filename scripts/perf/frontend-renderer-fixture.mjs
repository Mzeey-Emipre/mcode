...
async function installFixtureRuntime(page) {
  await page.evaluate(async () => {
    ...
    const providerCatalogModule = await import("/src/stores/providerCatalogStore.ts");
    ...
    const providerCatalogStore = providerCatalogModule.useProviderCatalogStore;
    ...
    const originalProviderCatalogLoad = providerCatalogStore.getState().load;
    providerCatalogStore.setState({
      load: (request, force) => {
        if (!request.threadId?.startsWith("perf-")) {
          return originalProviderCatalogLoad(request, force);
        }
        return Promise.resolve({
          providerId: request.providerId,
          context: request.workspaceId
            ? { scope: "workspace", workspaceId: request.workspaceId, threadId: request.threadId }
            : { scope: "user" },
...
export async function runRendererMatrix(page, runtime, sampleCount = 7, options = {}) {
  ...
  const markdownShiki = options.messageTimelineOnly ? { samples: [], checks: [] } : await timeFixture(page, sampleCount, async (sample) => {
    return page.evaluate(async (sampleIndex) => {
      const fixture = window.__issue1240;
      const revision = ++fixture.revision;
  ...
  const panelTransitions = options.messageTimelineOnly ? { samples: [], checks: [] } : await timeFixture(page, sampleCount, async (sample) => {
    return page.evaluate(async (sampleIndex) => {
      const fixture = window.__issue1240;
