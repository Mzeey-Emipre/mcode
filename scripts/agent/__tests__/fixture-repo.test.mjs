...
test("fixture repo seeds expected branches and real merge conflict", () => {
  ...
  try {
    ...
    const conflicted = git(fixture, ["diff", "--name-only", "--diff-filter=U"])
      .split(/\r?\n/)
      .filter(Boolean);
    assert.deepEqual(conflicted, ["agent.txt"]);
...
test("fixture seeding is idempotent inside .dev fixture directory", () => {
  const repo = makeHostRepo();
  try {
...
function makeHostRepo() {
  const dir = mkdtempSync(resolve(tmpdir(), "fixture-host-"));
...
function git(cwd, args) {
  return execFileSync("git", gitArgs(args), gitOptions(cwd));
}
...
function gitResult(cwd, args) {
  return spawnSync("git", gitArgs(args), gitOptions(cwd));
}
